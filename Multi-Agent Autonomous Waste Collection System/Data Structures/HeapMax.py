...
class Heapmax:
    ...
    Supports operations such as insertion, extracting the maximum element, 
    and increasing the key.
    """
    ...
    def __init__(self, vec:list[any], n:int) -> None:
        """
        Initializes a max-heap with n elements.
        ...
        # Build the max-heap by adjusting the elements.
    ...
    def extractMax(self) -> Qnode:
        """
        # Description
            -> Removes and returns the vertex with the highest key in the heap.

        := return: The vertex with the highest key.
        """
    ...
    def increaseKey(self, vertv:int, newkey:any) -> None:
        """
        # Description
            -> Increases the key associated with a vertex and adjusts its position in the heap.

        := param: vertv - The vertex whose key will be increased.
        ...
        # Adjust the position of the vertex while its key is greater than its parent's.
        while i > 1 and self.compare(i, self.parent(i)) > 0:
            self.swap(i, self.parent(i))
            i = self.parent(i)

    def insert(self, vertv:int, key:any):
        """
        # Description
        ...
        self.increaseKey(vertv, key)  # Fix the position of the vertex in the heap.
    ...
    def heapify(self, i:int) -> None:
        ...
        # Find the largest value between the current node and its children.
        if l <= self.size and self.compare(l, largest) > 0:
            largest = l
        if r <= self.size and self.compare(r, largest) > 0:
            largest = r

        # If the largest value is not the current node, swap and continue adjusting.
        if largest != i:
            self.swap(i, largest)
